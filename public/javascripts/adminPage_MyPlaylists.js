document.addEventListener('DOMContentLoaded', () => {
    ...
    var btn_getPlaylist = document.querySelectorAll('.btn_valPlaylist_admin_getPlaylist')
    for(let i = 0; i < btn_getPlaylist.length; i++) {
        btn_getPlaylist[i].addEventListener('click', async () => {
            window.location = "https://mytestferssite.ru"+btn_getPlaylist[i].getAttribute('btn_admin_getPlaylist')
        })
    }
